...
export function PlayerMapRoleProfilePanel({
  ...
  const columns: DataTableColumn<(typeof rows)[number]>[] = [
    ...
    { key: "position", label: "位置组", format: (row) => row.positionGroups[0]?.positionGroupId ?? "—" },
...
export function TeamMapRoleMatrixPanel({
  ...
  const columns: DataTableColumn<TeamMapRoleMatrix["players"][number]>[] = [
    ...
    { key: "position", label: "主要位置组", format: (row) => row.primaryPositionGroups.map((group) => `${group.positionGroupId} ${Math.round(group.share * 100)}%`).join(" · ") || "—" },

...
function row(roundNumber: number, overrides: Partial<TeamAwpRoundFact> = {}): TeamAwpRoundFact {
  return {
    analysisVersion: 2, matchId: `m${Math.ceil(roundNumber / 3)}`, mapName: roundNumber % 2 ? "de_nuke" : "de_inferno", roundNumber,
...
describe("double AWP analysis", () => {
  it("keeps CT qualified conditions, combinations and observable contribution fields", () => {
    ...
    expect(model).toMatchObject({ teamKey: "Team", side: "ct", status: "ready", doubleAwpRoundCount: 6, qualifiedRoundCount: 6, openingKills: 6, saves: 3, awpKills: 12, awpDamage: null });

...
- **批量导入卡死在「解析回合事件 5%」**：`export_demo` 在守护线程中调用
  `demoparser2` C 扩展时发生死锁（CLI 主线程/子进程下正常）。导出执行从
  `threading.Thread` 改为 `multiprocessing.Process`（与 CLI `export-batch`
  一致），子进程通过 `Queue` 回报进度。同时 `_safe_events` 异常回退路径现记录
  `WARNING` 日志便于诊断。

...
- **资料库**：导入 .dem / v2 ZIP（多选 / 拖拽 / 示例 / 批量标签）、检索、地图与标签筛选。ZIP 字节存 IndexedDB（内容哈希去重），.dem 经 exporter 转 ZIP 后入库（不存 .dem），解析结果只缓存内存。
...
数据库长期只存 v2 ZIP；.dem 在导入时即时转换：

- **开发模式**（`pnpm dev:studio`）：Vite 中间件 `POST /api/export-dem` 调 `uv run cs2dak export`。
- **打包版**（`scripts/package.sh`）：pywebview 桌面壳 `cs2dak-studio` 托管 Studio 构建产物，
  `.dem` 经 JS bridge（`python/src/cs2dak/studio.py`）走本机 exporter。Python 与前端打进同一个

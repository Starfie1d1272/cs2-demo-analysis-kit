  ...
  return (
    <>
      ...
      <button
        ...
        onDrop={(event) => { event.preventDefault(); event.stopPropagation(); if (event.dataTransfer.files.length > 0) void onImport(event.dataTransfer.files, context); }}
      >
        {label}
      </button>
    </>
  ...
  return (
    <section className="stu-event-stage" aria-labelledby={`stage-${eventId}-${stage.key}`}>
      <header className="stu-event-stage-head">
        ...
        {onImportOnlineFiles && stageCandidates.length > 0 && <ImportDropZone label="仅导入本阶段" secondary context={{ scope: "stage", eventId, candidates: stageCandidates }} onImport={onImportOnlineFiles} />}
      </header>
      ...
      <SeriesInspector series={selectedSeries} entries={entries} eventId={eventId} candidates={stageCandidates} onOpenMatch={onOpenMatch} onImportOnlineFiles={onImportOnlineFiles} />
    </section>
  );
}
...
export function EventsView({ entries, onOpenMatch, onAnalyzeEvent, onGoLibrary, rivalHubConnection, onConnectRivalHub, onRevokeRivalHub, onRefreshRivalHub, onImportOnlineFiles, refreshToken = 0 }: {
  ...
  return (
    <div className="stu-view stu-reading-view">
      ...
      {events.length === 0 ? <EmptyState title="还没有赛事目录" hint="连接 RivalHub 获取在线赛事，或在资料库导入本地 event-package/1.0 资源包。" action={<button className="stu-button" onClick={onGoLibrary}>去资料库</button>} /> : <div className="stu-event-directory">
        ...
        {active && <section className="stu-event-content"><header className="stu-event-content-head"><div><h2>{active.name}</h2><p>{active.kind} · {eventSeries.length} 个系列 · {active.source}{active.readOnly ? " · 只读资产" : ""}</p><div className="stu-event-sync-summary"><span>本地已关联 {localLinked}/{totalMaps} 图</span>{active.source === "rivalhub" && <><span>远程已同步 {remoteSynced}/{eventCandidates.length} 图</span><span>需要处理 {remoteNeedsAttention}</span>{active.rivalHub?.lastSyncedAt && <span>上次同步 {new Date(active.rivalHub.lastSyncedAt).toLocaleString("zh-CN")}{active.rivalHub.stale ? " · 缓存已过期" : ""}</span>}</>}</div></div><button type="button" className="stu-button" onClick={() => onAnalyzeEvent(active)}>查看赛事总览</button></header>{onImportOnlineFiles && active.source === "rivalhub" && eventCandidates.length > 0 && <ImportDropZone label="将本赛事 Demo 拖到这里；可混合排位赛 / 正赛，自动匹配。" context={{ scope: "event", eventId: active.id, candidates: eventCandidates }} onImport={onImportOnlineFiles} />}{stageOptions.length > 0 && <div className="stu-stage-tabs" role="tablist" aria-label="赛事阶段">{stageOptions.map((stage) => <button key={stage.key} type="button" role="tab" aria-selected={activeStage?.key === stage.key} className={activeStage?.key === stage.key ? "stu-chip stu-chip-active" : "stu-chip"} onClick={() => setActiveStageKey(stage.key)}>{stage.name}</button>)}</div>}{activeStage && <EventStageSection eventId={active.id} stage={activeStage} series={activeStageSeries} entries={entries} candidates={eventCandidates} remote={active.source === "rivalhub"} onOpenMatch={onOpenMatch} onImportOnlineFiles={onImportOnlineFiles} />}</section>}
      </div>}
    </div>
  );
}

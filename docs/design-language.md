...
- 禁止组件内联 style 写颜色/字体（布局尺寸类内联可接受）；禁止每个视图
  自造一套按钮/chip/表格样式——公共原语（按钮、tab、chip、stat 卡、
  数据表、空态、ⓘ tooltip）只在 studio.css 公共段定义一次。

## 4. 交互规范

1. **Query-first 证据链**：凡跨页跳转到 2D 回放/回合列表的统计值，渲染为
   accent 色可点元素（统一的「证据链接」原语），hover 显示去向。
2. **ⓘ 口径说明**：所有派生指标必须带 ⓘ tooltip（公式、窗口参数、已知误差）。
   宁缺毋滥：做不准的指标标 beta 或不展示，不给黑箱数。
3. **空态三件套**：每个视图必须处理 无数据 / 数据不足（样本量过小）/
   加载中 三种状态，使用公共空态原语，文案说明「需要什么才能点亮本页」。

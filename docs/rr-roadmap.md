...
| UtilitySpatial actual-effect（手雷 zone 归属 + 火焰逼退/烟雾隔离/路径延迟） | ✅ `core/spatial/utility.ts`（4 图归属率 94–98%）；review/shadow，未进 RR |
| UtilitySpatial 的 smoke crossing / sightline denial | ⬜ 需静态 LOS（tri-BVH）接线判定「无烟则暴露」 |

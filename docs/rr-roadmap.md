...
| SP2 `strategicIsolationDeaths` → Trade 闭环 | ✅ `core/spatial/mapcontrol.ts`（activeSoloPressure MVP gates）接进 `trade`；rival-rating 已就绪自动激活 |
| 空间账户其余 official gates（denial/advance/firstControl + UtilitySpatial actual-effect） | ⬜ SP3（denial/advance 需 nav 距离/LOS；UtilitySpatial 需 zone 多边形标定） |

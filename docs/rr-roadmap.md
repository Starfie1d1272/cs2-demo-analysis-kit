...
`.dem` 导出端是 `cs2df`；本 kit 的 TS 侧负责**信号派生 + 接线 + 锚定 + 展示**。

## 数据现状（55 场锦标赛，仅差总决赛）

- 55 场 demo 需要以 `cs2-demo-format/3.x` 重导后进入当前管线；富事件 + replay/shots/duels 见 [architecture](architecture.md)。
- **per-match 的 HLTV baseline / RR 六账户现在就能跑全 55 场。**
...
让当前 ZIP 产物可信、可解释，不引入新能力。

- **missing vs zero**：context 分桶在数据源缺失时发 `null`（模型降级为乘子 1.0），不再混成零桶。
- **账户 breakdown 透传**：模型已算出的六账户贡献，落到 scoreboard / view-model（此前被丢弃）。
...
详见 [rr-model.md](design/rr-model.md)。
...
玩家向整体方案见 [rr-model.md](design/rr-model.md)「当前方案 vs 未来方案」。

- **R0**：把 cohort z-score 换成 normalizer strategy 接口（`cohort_normalizer` / `frozen_pro_baseline_normalizer`），接口先用现有 55 场跑通。
- **R1**：用 Phase D1 职业样本算出六账户分布，percentile mapping / sigmoid 曲线，冻结进 `rr-v2-pro-baseline.json`。单 demo 即可出绝对 RR，不依赖 cohort。
...
## 阶段 4 — 地图语义层 + 空间分析（设计见 [rr-model.md](design/rr-model.md)）
...
```text
.dem
 └─ cs2df ──► cs2-demo-format/3.x ZIP（富事件 + replay/shots/duels）
                                     │
              ┌──────────────────────┴───────────────────────┐
              ▼                                               ▼
...
|---|---|
| RR v1（box-score） | ✅ 可用 |
| RR 六账户 | ✅ 可用（权重为未校准先验；MapControl/Utility spatial 需空间派生接入） |
| buyDelta / manState context | ✅ 已实现并接线（阶段 0 补了 null 语义） |
| clutch 超额（实际−期望） | ✅ 已实现（无 shrinkage，见 rr-model.md） |
...
| 7 图动线 + callout 中文映射 | ✅ `@cs2dak/maps`（`MapRoute` + `CALLOUT_NAME_CN`） |
| 7 图 compact nav（BFS/nearestArea/pathLength） | ✅ `map-nav/*.nav.json`（6.1MB）+ `nav.ts` |
| awpy tri + BVH + staticLineOfSight | ✅ `visibility.ts`（几何地基保留） |
| route-callout 空间分析（**proxy**） → MapControl + UtilitySpatial | 🟥 proxy 实现已移除；MapControl/UtilitySpatial 当前发 **shadow null**（不进 RR 总分），几何地基保留待严格重建 |
| SP1 phase 模型 + 位置标注层 | ✅ `core/spatial/{phase,annotate}.ts`（freeze/take/execute/postPlant/clutch + callout/nav 标注） |
| SP2 `strategicIsolationDeaths` → Trade 闭环 | ✅ `core/spatial/mapcontrol.ts` 接进 `trade`；rival-rating 已就绪自动激活 |
| official MapControl 派生：soloPressure / denial / firstControl | ✅ callout/index-based MVP gates（review/shadow + isolation 触发器；未进 RR 评分） |
| 其余 official gates：nonUtilityAssistedAdvance + denial 的 nav 距离/LOS 精修 | ⬜ 需 nav 距离函数 + LOS 接线（+ utility window 降权依赖 zone） |
| zone 多边形标定 | 🟡 5/7 图已标定（ancient/anubis/dust2/inferno/mirage，覆盖 43/55 NJU 场）；nuke/overpass 待标 |
| UtilitySpatial actual-effect 全 5 项（手雷 zone 归属 + 火焰延迟/逼退 + 烟雾隔离 + LOS 视线封锁/掩护穿越） | ✅ `core/spatial/{utility,utility-geometry}.ts`（4 图，tri-BVH + nav 绕路）；review/shadow，未进 RR |
| UtilitySpatial 质量缺口 | 🧊 **已冻结（2026-06-07）**：净 ΔRR 极小（mean 0.011），保持 shadow 不再打磨。隔离秒 nav 绕路开阔图恒 0（烟不挡移动）+ dust2 视线偏低（objective 选点）= 模型固有缺陷，已在 `utility.ts` 钉注释。vision-based 重构推迟到阶段三权重 ramp |
| MapControl 评分账户进 RR（schema V2 重命名 + 权重 ramp） | ⬜ SP4（跨仓库 rival-rating；当前 proxy 字段发 null = de facto shadow） |
| 区域多边形标定 / Area v1 / Aim v1 | ⬜ 阶段 4（标定待人工，计算设计见 rr-model.md §3） |
